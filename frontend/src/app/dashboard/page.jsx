...
const menuItems = [
  { name: "Overview", href: "/dashboard", icon: <Image src="/chart.svg" alt="chart" width={20} height={20} /> },
  { name: "Product List", href: "/productList", icon: <ListChecks className="w-5 h-5" /> },
  { name: "Add Product", href: "/dashboard/tasks", icon: <ClipboardPlus className="w-5 h-5" /> },
  { name: "Update Product", href: "/dashboard/members", icon: <Scissors className="w-5 h-5" /> },
];

const workplaceItems = [
  { name: "About Us", href: "/dashboard/settings", icon: <Trophy className="w-5 h-5" /> },
  { name: "Log out", href: "/logout", icon: <Image src="/logout.svg" alt="logout" width={20} height={20} /> },
];

export default function DashboardLayout({ children }) {
  const router = useRouter();
  const pathname = usePathname();
  const [isAuthenticated, setIsAuthenticated] = useState(null); // null = en cours, false/true ensuite
